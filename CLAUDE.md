...
`src/lib/session.ts`'s `resolveUserId()` is the single source of truth for "who is the current user" everywhere in the app (page guard, dashboard layout, every server action, the Blackboard sync route) — it's what all of those call instead of `auth()` directly. It also honors `AUTH_DEV_BYPASS=true` (`.env`, non-production only): when set and no real NextAuth session exists, it skips NextAuth/Microsoft entirely and returns a fixed local `dev@localhost` user, auto-created in the DB. This exists because Auth.js's Credentials provider always issues JWT-encoded session cookies regardless of the configured strategy, which is incompatible with this app's database-session setup — so the bypass lives outside NextAuth's session machinery rather than as an extra provider. A real NextAuth session always takes priority over the bypass (checked first) — this is what lets the "Connect Microsoft account" button on Settings actually work while bypassed: once you complete the real OAuth flow, `auth()` returns a real session and the app starts using that user (with its real `Account` row) instead of the bypass user, until you sign out again. Mail sync won't work under the bypass user itself (no real Microsoft `Account` row).
